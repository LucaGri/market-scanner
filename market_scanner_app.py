...
FTSE_MIB = ['UCG.MI', 'ISP.MI', 'ENI.MI', 'ENEL.MI', 'TIT.MI', 'STM.MI', 'G.MI', 
            ...
            'BAMI.MI', 'DIA.MI', 'FBK.MI', 'HER.MI', 'IP.MI', 'LDO.MI',
...
markets = st.sidebar.multiselect(
    ...
    default=["FTSE MIB", "DAX", "S&P 500 Large Caps"]
)
...
def download_stock_data(ticker, period="60d"):
    """Scarica dati storici per un ticker"""
    try:
...
def calculate_compression_score(df):
    ...
    # Verifica se il prezzo è in un range stretto per più giorni
...
if st.button("🚀 Avvia Scan", type="primary", use_container_width=True):
    ...
    if results:
        ...
        **Score < 60:** ℹ️ Compressione debole - Bassa priorità
        ...
        st.info("💡 Prova ad abbassare il 'Score minimo' nella sidebar.")
